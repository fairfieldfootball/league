...
class Client extends ApiClient<ClientApi, User> {
  ...
  get api(): ClientApi {
    return {
      ...
      updateUser: data => this.do(this.adminApiV1('/user'), { method: HttpMethods.Put, body: data }),
      yahoo: () => ({
        authenticate: (code: string) =>
          this.do(this.adminApiV1('/yahoo/session'), { method: HttpMethods.Post, body: { code } }),
        logout: () => this.do(this.adminApiV1('/yahoo/session'), { method: HttpMethods.Delete }),
        game: (game: 'nfl') => this.do(this.adminApiV1(`/yahoo/games/${game}`), { method: HttpMethods.Get }),
      }),
    };
  }
}
